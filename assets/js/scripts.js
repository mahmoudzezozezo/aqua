...
function renderMainTable(lang) {
  ...
  const headers = isAr
    ? ['#','التاريخ والوقت','القراءة','الفارق (س)','المعدل (م³/يوم)','التكلفة']
    : ['#','Date & Time','Reading','Diff (hrs)','Avg (m³/day)','Est. Cost'];
...
function isAuthenticated() {
  return sessionStorage.getItem("water_auth") === "true";
}
...
function checkPassword(event) {
  ...
  if (pwd === MANAGER_PASSWORD) {
    sessionStorage.setItem("water_auth", "true");
